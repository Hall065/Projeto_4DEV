...
export default function SafeApprovalsScreen() {
  ...
  const decide = useSafeStore((state) => state.decideAsProfessor);

  useFocusEffect(useCallback(() => { void loadAuthorizations({ status: 'aguardando_professor' }); }, [loadAuthorizations]));

  const act = async (id: string, approve: boolean) => {
    const confirmed = await confirm({
      title: approve ? 'Aprovar solicitacao' : 'Negar solicitacao',
      message: approve
        ? 'A entrada sera finalizada imediatamente; uma saida seguira para a portaria.'
  ...
  return (
    ...
    >
      ...
      <SurfaceCard title="Aguardando sua decisao" subtitle={String(authorizations.length) + ' solicitacao(oes)'}>
        {authorizations.map((item) => (
          <View key={item.id}>
            <SafeAuthorizationRow item={item} />
            <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12 }}>
              <AppButton label="Negar" variant="secondary" accent={colors.red} icon={<X size={15} color={colors.red} />} onPress={() => void act(item.id, false)} loading={submittingId === item.id} wrapperStyle={{ flex: 1 }} />
              <AppButton label="Aprovar" accent={colors.green} icon={<Check size={15} color={colors.white} />} onPress={() => void act(item.id, true)} loading={submittingId === item.id} wrapperStyle={{ flex: 1 }} />
            </View>
          </View>
        ))}
      </SurfaceCard>
